...
{
	return fputs(source, (FILE*) handle) >= 0;
}
 ...
 */
	...
	if (get_sourcefile(1) != NULL)
	{
		...
		for (fidx = 0; (sname = get_sourcefile(fidx)) != NULL; fidx++)
		{
			...
			pc_writesrc(ftmp, "#file \"");
			pc_writesrc(ftmp, sname);
			pc_writesrc(ftmp, "\"\n#line 0\n");
...
static cell needsub(char match, constvalue **namelist)
{
	...
	if (namelist != NULL && matchtoken(tSYMLABEL)) {
		...
		symbol *sym;
		int exprtag;
		constexpr_(&val, &exprtag, &sym);/* get value (must be constant expression) */
		if (tag == 0)
...
static void decl_const(int scope)
{
	...
	do {
		...
		do {
			...
			if (enumerate) {
				if (matchtoken('='))
					constexpr_(&val, &exprtag, NULL);/* get value (optional, except for the first) */
				else if (first)
...
static void make_report(symbol *root, FILE *log, const char *sourcefile, int *makestategraph)
{
	...
	ptr = strrchr((char*) sourcefile, DIRSEP_CHAR);
	if (ptr != NULL)
		ptr++;
	else
		ptr = (char*) sourcefile;
...
static void reduce_referrers(symbol *root)
{
	...
	do
	{
		...
		for (sym = root->next; sym != NULL; sym = sym->next)
		{
			...
			if  (sym->ident == iFUNCTN &&
				(sym->usage & uNATIVE) == 0 && (sym->usage & uPUBLIC) == 0 &&
